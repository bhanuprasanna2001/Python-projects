...
class TaskRepository:
    ...
    def create(self, task: Task) -> Task:
        ...
        with self._connection() as conn:
            cursor = conn.execute(
                """
                INSERT INTO tasks (title, description, is_completed, created_at, priority, due_date, category)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    task.title,
    ...
    def update(self, task: Task) -> bool:
        ...
        with self._connection() as conn:
            cursor = conn.execute(
                """
                UPDATE tasks 
                SET title = ?, description = ?, is_completed = ?, priority = ?, due_date = ?, category = ?
                WHERE id = ?
                """,
                (
                    task.title,
    ...
    def find_by_date_range(
        ...
        if field not in ("due_date", "created_at"):
            raise ValueError(f"Invalid date field: {field}")
        
        with self._connection() as conn:
            rows = conn.execute(
                f"SELECT * FROM tasks WHERE {field} >= ? AND {field} <= ? ORDER BY {field}",
    ...
    def get_sorted(self, sort_by: str) -> Sequence[Task]:
        ...
        if sort_by == "priority":
            order_clause = """
                ...
                END, due_date
            """
        elif sort_by in ("due_date", "created_at"):
        ...
        with self._connection() as conn:
            rows = conn.execute(
                f"SELECT * FROM tasks ORDER BY {order_clause}"
            ).fetchall()
        return [self._row_to_task(row) for row in rows]
